// ==UserScript==
// @name         Cashnote Coupon Extractor
// @namespace    http://tampermonkey.net/
// @version      6.2
// @description  캐시노트 쿠폰 추출기 (이미지 분석 AI 버전) - UserScript
// @author       krazyeom
...
(function() {
    ...
    const initInterval = setInterval(() => {
        ...
        if (document.getElementById('cashnote-userscript-btn')) return;

        // Create floating button
        const triggerBtn = document.createElement('div');
        triggerBtn.id = 'cashnote-userscript-btn';
        triggerBtn.style.cssText = 'position:fixed; bottom:30px; right:30px; background:#0f0; color:#000; padding:15px 25px; border-radius:30px; font-weight:bold; font-size:15px; cursor:pointer; z-index:99999; box-shadow:0 5px 15px rgba(0,255,0,0.3); font-family:sans-serif; transition:transform 0.2s, background 0.2s;';
        triggerBtn.innerHTML = '🔍 쿠폰 추출';
        triggerBtn.onmouseover = () => { triggerBtn.style.transform = 'scale(1.05)'; triggerBtn.style.background = '#0d0'; };
        triggerBtn.onmouseout = () => { triggerBtn.style.transform = 'scale(1)'; triggerBtn.style.background = '#0f0'; };
        ...
        triggerBtn.onclick = async () => {
            ...
            const showAboutModal = () => {
                ...
                card.innerHTML = `
                    ...
                    <div style="font-size:14px; color:#ccc; line-height:1.6;">
                        <strong>Version:</strong> v6.2 (UserScript)<br>
                        ...
                        <div style="border-top:1px solid #222; padding-top:10px;">
                            ...
                            • v6.2: Migrated to UserScript format.<br>
            ...
            try {
                ...
                const scanMoreBtn = document.createElement('span');
                scanMoreBtn.style.cssText = 'cursor:pointer; color:#fa0; margin-right:15px; display:none; font-size:13px; font-weight:bold;';
                scanMoreBtn.textContent = '[+5 PAGES]';
                historyBtn.parentElement.insertBefore(scanMoreBtn, historyBtn);
                ...
                const renderResults = () => {
                    ...
                    targetKeywords.forEach(k => {
                        const items = sortedResults.filter(i => i.category === k);
                        if (items.length > 0) {
                            detailedOutput += '[' + k + ']\n';
                            items.forEach(i => {
                                let icon = (i.status === 'UNUSED') ? '✅' : (i.status === 'USED' ? '➖' : (i.status === 'CANCELED' ? '❌' : '⚠️'));
                    ...
                    targetKeywords.forEach(k => {
                        ...
                        const catUnused = sortedResults.filter(i => i.status === 'UNUSED' && i.category === k);
                ...
                const scanOrders = async (pagesToScan) => {
                    const startPage = page;
                    container.style.display = 'none';
                    while (hasMore && page < startPage + pagesToScan) {
